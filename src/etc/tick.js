...
function tick(diff){
    ...
    // Automation Tier 2
    // BuyMax Autobuyer
    if(timesToLoop[2].gte(1) && (D(data.markup.powers).lt(getFSReq()) || data.ord.base === 3 || data.baseless.baseless) && data.autoStatus.enabled[0]){
        buyMaxT1()
    }

    // Markup Autobuyer
    let collapseCheck = data.ord.ordinal.lt(BHO_VALUE) || data.collapse.times > 0 || data.ord.base != 3
