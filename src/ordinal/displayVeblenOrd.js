...
function displayVeblenOrd(ord,over,base,trim = data.ord.trim,forcePsi = false) {
    if(data.ord.isPsi || forcePsi) return displayPsiVeblenOrd(ord, trim, base)
...
function displayInfiniteVeblenOrd(ord, over, base, trim = data.ord.trim, recursionDepth = 0){
    ...
    if (D(ord).gte(Decimal.tetrate(base,Math.min(Math.max(data.ord.trim+1,data.ord.base),maxRecursionDepth)))) {
        ...
        return "&phi;<sup>" + displayInfiniteVeblenOrd(nLayer, over, base, trim, recursionDepth) + "</sup>(1)"
    }
...
function displayBase2PsiVeblenOrd(ord, trim = data.ord.trim) {
    ...
    if (ord.gte(D("(e^252)619.299370844483"))) veblenOutput = "&phi;(1@(1,0)<sub>I<sub>&phi;(1)</sub></sub>)"; // ψ(I_ω) level
    if (ord.gte(D("(e^253)619.299370844483"))) veblenOutput = "&phi;(1@(1,0)<sub>I<sub>(1,0)</sub></sub>)"; // ψ(I_Ω) level
    if (D(ord.layer).gte(D("(e^252)619.299370844483"))) veblenOutput = "&phi;(1@(1,0)<sub>I<sub>(1,1)</sub></sub>)"; // ψ(I_(Ω+1)) level
    if (D(D(ord.layer).layer).gte(D("(e^252)619.299370844483"))) veblenOutput = "&phi;(1@(1,0)<sub>M</sub>)";; // ψ(I_I) = ψ(M) level
    return veblenOutput
}
