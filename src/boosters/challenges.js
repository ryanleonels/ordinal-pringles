...
function updateChalHTML(i){
    ...
    DOM(`chal7`).innerHTML = `Challenge 8<br>${chalDesc[7]}<br><br>Goal: ${format(chalGoals[7][data.chal.completions[7]])} OP<br>Reward: ${data.base2 ? 'Factor 8 and ' : ''}Dynamic Factor slightly boosts Tier 2 Automation<br>Completions: ${data.chal.completions[7]}/3`
}
function chalEnter(i, force=false){
    if(((data.chal.completions[i] === 3 && !data.enterChalAfter3) || data.chal.active.includes(true)) && !force) return
